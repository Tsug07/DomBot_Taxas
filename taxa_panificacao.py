"""
DomBot - Taxa Panificação
=======================
Automação RPA para emissão de relatórios de Taxa Panificação no sistema Domínio Folha.

As empresas são cadastradas e gerenciadas pela própria interface (aba "🏢 Empresas")
e ficam salvas automaticamente em 'empresas_panificacao.json' ao lado deste script.
...
class AutomacaoGUI:
    ...
    def __init__(self):
        ...
        self.window = ctk.CTk()
        self.window.title("DomBot - Taxa Panificação v1.0")
        ...
        self.empresas_json = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "empresas_panificacao.json"
        )
        ...
        self.logger = logging.getLogger('AutomacaoTaxaPanificacao')
    ...
    def setup_file_logging(self):
        ...
        self.success_logger = logging.getLogger('SuccessLogTaxaPanificacao')
        ...
        self.error_logger = logging.getLogger('ErrorLogTaxaPanificacao')
    ...
    def iniciar_automacao(self):
        ...
        try:
            ...
            for idx, emp in enumerate(empresas_processar):
                ...
                try:
                    success = automacao.processar_taxa_panificacao(
...
class DominioAutomation:
    ...
    def publicar_documento(self, nome_arquivo: str) -> bool:
        """
        [FORA DO FLUXO ATUAL — mantido apenas como referência]
        Este método publicava por-empresa via a janela 'Publicação de Documentos', mas
        essa tela não permite definir data de validade. A publicação passará a ser feita
        em lote no final da execução, por outra tela do Domínio. Não é mais chamado por
        processar_taxa_panificacao.

        Publica o relatório no portal Onvio via a janela 'Publicação de Documentos'.
        Abre pelo botão-imagem (id 1000) da tela de visualização do relatório, define a
        pasta (combobox 1007), o nome do arquivo (edit 1014), garante o checkbox
        'Pasta padrão' (1008) marcado e clica em 'Gravar' (1016).
        Ao final (sucesso ou falha) limpa as janelas com cleanup_windows().
        """
        try:
            if self.should_stop():
                return False
            ...
            self.log("🌐 Iniciando publicação no portal Onvio")

            # 1) Clicar no botão que abre a janela de publicação (id 1000)
            aberto = False
            for tentativa in range(3):
                if self.should_stop():
                    return False
                try:
                    btn_pub = self.main_window.child_window(auto_id="1000", class_name="Button")
                    if btn_pub.exists():
                        btn_pub.click_input()
                        aberto = True
                        break
                except Exception as e:
                    self.log(f"⚠️ Tentativa {tentativa + 1} de abrir publicação falhou: {e}")
                if not self.smart_sleep(1):
                    return False

            if not aberto:
                self.log("❌ Botão de publicação (id 1000) não encontrado")
                self.cleanup_windows()
                return False

            # 2) Aguardar a janela 'Publicação de Documentos' (class FNWNS3190)
            if not self.wait_for_condition(
                lambda: self._window_exists("Publicação de Documentos", "FNWNS3190"),
                timeout=30, poll_interval=0.3,
                description="Aguardando janela de Publicação de Documentos",
            ):
                self.log("❌ Janela 'Publicação de Documentos' não apareceu")
                self.cleanup_windows()
                return False

            pub = self.main_window.child_window(
                title="Publicação de Documentos", class_name="FNWNS3190"
            )
            pub_hwnd = self._find_publicacao_window_hwnd()

            if self.should_stop():
                return False
            self.check_pause()

            # 3) Definir a pasta no combobox (id 1007)
            self.log(f"📁 Definindo pasta: {self.PASTA_PUBLICACAO}")
            try:
                combo = pub.child_window(auto_id="1007", class_name="ComboBox")
                combo.set_edit_text(self.PASTA_PUBLICACAO)
            except Exception as e:
                self.log(f"⚠️ set_edit_text na pasta falhou ({e}), usando clipboard")
                if pub_hwnd:
                    self._preencher_campo_por_clipboard(pub_hwnd, "ComboBox", self.PASTA_PUBLICACAO)
            if not self.smart_sleep(0.4):
                return False

            # 4) Definir o nome do arquivo (edit id 1014) via clipboard (acentos/espaços)
            self.log(f"📝 Definindo nome do arquivo: {nome_arquivo}")
            preenchido = False
            if pub_hwnd:
                preenchido = self._preencher_campo_por_clipboard(pub_hwnd, "Edit", nome_arquivo)
            if not preenchido:
                try:
                    edit_nome = pub.child_window(auto_id="1014", class_name="Edit")
                    edit_nome.set_edit_text(nome_arquivo)
                except Exception as e:
                    self.log(f"⚠️ Não foi possível definir o nome do arquivo: {e}")
            if not self.smart_sleep(0.3):
                return False

            # 5) Garantir o checkbox 'Pasta padrão' (id 1008) marcado
            try:
                chk = pub.child_window(auto_id="1008", class_name="Button")
                if chk.exists():
                    estado = None
                    try:
                        estado = chk.get_toggle_state()  # 1 = marcado, 0 = desmarcado
                    except Exception:
                        estado = None
                    if estado == 0:
                        self.log("☑ Marcando 'Pasta padrão'")
                        chk.click_input()
                        self.smart_sleep(0.3)
            except Exception as e:
                self.log(f"⚠️ Não foi possível verificar 'Pasta padrão': {e}")

            # 6) Tratar diálogos de erro que possam ter surgido
            ...
            # 7) Clicar em 'Gravar' (id 1016) para publicar
            self.log("💾 Gravando publicação no portal")
            try:
                btn_gravar = pub.child_window(auto_id="1016", class_name="Button")
                btn_gravar.click_input()
            except Exception as e:
                self.log(f"⚠️ Botão 'Gravar' não encontrado, tentando Alt+G: {e}")
                send_keys('%g')

            # 8) Aguardar a janela de publicação fechar, tratando diálogos no caminho
            inicio = time.time()
            while time.time() - inicio < 60:
                ...
                if self._any_error_dialog_visible():
                    if not self.handle_error_dialogs():
                        self.cleanup_windows()
                        return False
                if not self._window_exists("Publicação de Documentos", "FNWNS3190"):
                    break
                time.sleep(0.3)

            self.log("✅ Documento publicado no portal Onvio")
            self.cleanup_windows()
            return True

        except Exception as e:
            self.log(f"❌ Erro ao publicar documento: {e}")
            self.cleanup_windows()
            return False

    def _find_publicacao_window_hwnd(self) -> int:
        """Localiza o hwnd da janela 'Publicação de Documentos' (class FNWNS3190)."""
        result = [0]
        def cb(hwnd, _):
            if win32gui.IsWindowVisible(hwnd):
                try:
                    if (win32gui.GetClassName(hwnd) == "FNWNS3190"
                            and win32gui.GetWindowText(hwnd) == "Publicação de Documentos"):
                        result[0] = hwnd
                        return False
                except Exception:
                    pass
    ...
    def _preencher_campo_por_clipboard(self, parent_hwnd: int, edit_class: str, texto: str) -> bool:
        """
        Preenche um campo (Edit/ComboBox) filho de parent_hwnd colando via clipboard,
        seguindo o mesmo idioma usado em salvar_pdf (foco + Ctrl+A + Ctrl+V).
        """
        try:
            self._force_focus(parent_hwnd)
            self._set_clipboard(texto)
            campo = win32gui.FindWindowEx(parent_hwnd, 0, edit_class, None)
            if not campo:
                return False
            win32gui.SetFocus(campo)
            time.sleep(0.1)
            send_keys('^a')
            time.sleep(0.1)
            send_keys('^v')
            ...
            self.log(f"⚠️ Falha ao preencher campo {edit_class} por clipboard: {e}")
            return False
    ...
    def processar_taxa_panificacao(self, rubrica: str, codigo: str, nome: str,
                                  caminho: str, competencia: str, linha_excel: int) -> bool:
        """
        Abre Relatórios > Gerenciador > pasta Diversos (d x6 + Enter) > 'c' x9 para
        selecionar a guia Taxa Panificação, preenche competência e rubrica, executa e salva.
        """
        try:
            ...
            # Pressionar 'c' 9 vezes para selecionar a guia Taxa Panificação
            self.log("🎯 Selecionando guia Taxa Panificação (c x9)")
            for _ in range(9):
                if self.should_stop():
                    return False
                send_keys('c')
            ...
            # Apenas salva o PDF localmente. A publicação será feita em lote no final
            # (outra tela do Domínio, com data de validade), varrendo a pasta de saída.
            # O método publicar_documento() foi mantido apenas como referência.
